...
The data must be formatted so that it includes:
    x: observations of the newly created links, in sequence
    n: observations of the network when each link is created, in sequence
...
For n, we use a dictionary of timestamps and network statistics at time t, '{t: [Pa_tot, Pb_tot, Ua_tot, Ub_tot]}. 'We ommit the notation of t, but all statistics here are temporal. In our likelihood functions the probability of creating a link from group a to b, where b is of degree k is written as:
p^{ab}_k = (c*(n^b_k*k)/(sum_j(n^a_j*j+n^b_j*j))+ (1-c)*(n^b_k)/(sum_j(n^a_j+n^b_j)). Where n_k is the same as tgt_cnt, and k is tgt_k. 
The network statistics for n are then we use are then Pa_tot=sum_j(n^a_j*j), Pb_tot=sum_j(n^b_j*j), Ua_tot = sum_j(n^a_j) and Ub_tot = sum_j(n^b_j), 
